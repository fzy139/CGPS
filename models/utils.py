...
from models.data import recover_image,normalize
...
def channel_grouping_patching_swapping(content_feature, style_feature, content_surface,
...
def display(output_image, title='', pure = False, show=True, save=False, out_dir='./'):
    output_image = recover_image(output_image).numpy()
    output_image = np.clip(output_image / np.max(output_image) * 255, 0, 255).astype(np.uint8)
    print('Save at' + out_dir)
    if pure:
        Image.fromarray(output_image).save(out_dir + '/' + title + '.bmp', format='bmp')
    if show:
        plt.axis('off')
        plt.title(title)
