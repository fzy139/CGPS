...
from models.utils import Conv2DResize, Conv2DSame, channel_grouping_patching_swapping
from models.data import get_guiding_image
...
def decode_with_skip_connections(decoder, swapped_feature, style_feature_list):
    mean_input = []
    sigma_input = []
...
def forward(encoder, decoder,
    ...
    # get guiding image
    lc = get_guiding_image(content, img_filter, filter_paras)

    # get feature map and its low pass as surface v
    content_feature = encoder(content)[-1]
    style_feature_list = encoder(style)
    style_feature = style_feature_list[-1]
    content_surface = encoder(lc)[-1]

    # transfer
    recon_feature = channel_grouping_patching_swapping(content_feature, style_feature, content_surface, preserve_content,mask,ratio)

    # decode
    output = model.decode_with_skip_connections(decoder, recon_feature, style_feature_list)
    return output,
